...
function gymTableSVG() { return `<svg viewBox="0 0 44 22" preserveAspectRatio="none"><ellipse cx="22" cy="20" rx="20" ry="1.8" fill="#000" fill-opacity=".22"/><rect x="4" y="16" width="2.4" height="5" fill="#14242f"/><rect x="37.6" y="16" width="2.4" height="5" fill="#14242f"/><rect x="2" y="2" width="40" height="16" rx="1" fill="#0d3a5c"/><rect x="3" y="3" width="38" height="14" fill="#1f6fb2"/><rect x="3" y="3" width="38" height="14" fill="none" stroke="#eef3f7" stroke-width="0.8"/><rect x="3" y="9.6" width="38" height="0.8" fill="#eef3f7"/><rect x="21.4" y="2.4" width="1.2" height="15.2" fill="#f4f8fb"/><rect x="21.2" y="1.6" width="1.6" height="1.2" fill="#cdd6df"/><rect x="21.2" y="17.2" width="1.6" height="1.2" fill="#cdd6df"/><text x="11" y="7.4" font-size="3" fill="#bfe0f5" font-family="Arial" font-weight="bold" text-anchor="middle">ALMAR</text><text x="33" y="14.4" font-size="3" fill="#bfe0f5" font-family="Arial" font-weight="bold" text-anchor="middle">ALMAR</text><circle cx="30" cy="7" r="1.3" fill="#fff"/></svg>`; }
...
function gymStageHtml(layout, t, editable) {
  ...
  (layout.props || []).forEach((p, i) => { const sp = GYM_SPAN[p.type] || [2, 1]; const sprite = (GYM_SPRITE[p.type] || (() => ''))();
    els += `<div class="gym-el g-prop" ${editable ? `data-kind="prop" data-idx="${i}" data-sx="${sp[0]}" data-sy="${sp[1]}"` : ''} style="${pos(p.x, p.y, sp[0], sp[1])}"><div class="g-cap">${GYM_CAP[p.type] || ''}</div>${sprite}${p.type === 'buffet' ? '<div class="smoke"><span></span><span></span><span></span></div>' : ''}${(!editable && p.type === 'stands') ? '<div class="cheer c1">¡Vamos! 🏓</div><div class="cheer c2">¡Dale campeón!</div><div class="cheer c3">Apurá 😤</div>' : ''}${editable ? `<button class="g-del" onclick="gymDelProp(${i})" title="Quitar">✕</button>` : ''}</div>`; });
  (layout.tables || []).forEach((tb, i) => { const vert = tb.rot === 90, sw = vert ? 1 : 2, sh = vert ? 2 : 1, num = i + 1, o = occ[num];
    els += `<div class="gym-el g-table ${o ? 'busy' : 'free'}" ${editable ? `data-kind="table" data-idx="${i}" data-sx="${sw}" data-sy="${sh}"` : ''} style="${pos(tb.x, tb.y, sw, sh)}"><div class="g-trot" style="transform:rotate(${tb.rot || 0}deg)">${gymTableSVG()}</div><div class="tnum">${num}</div>${o ? `<div class="g-occ"><span class="g-live-dot"></span>${esc(o.title)}</div>${o.sub ? `<div class="g-occ-sub">${esc(o.sub)}</div>` : ''}` : (editable ? '' : `<div class="g-occ dim">libre</div>`)}${editable ? `<button class="g-del" onclick="gymDelTable(${i})" title="Quitar mesa">✕</button>` : ''}${(!editable && freed.includes(num)) ? gymBurstHtml() : ''}</div>`; });
  return `<div class="gym-stage ${editable ? 'editing' : ''} ${(!editable && freed.length) ? 'celebrate' : ''}" id="gymStage">${els}</div>`;
}
