...
// Tribuna: el espectador tiene TAMAÑO CONSTANTE — al agrandar la tribuna entran MÁS personas (no se
// agrandan). viewBox proporcional a w×h (preserveAspectRatio none → 1 unidad = tamaño fijo en pantalla).
function gymStandsSVG(w, h) {
  ...
  const VW = Math.max(24, Math.round((w || 3) * 12)), VH = Math.max(14, Math.round((h || 2) * 12)), cw = 6.4, rh = 7.4;
  const cols = Math.max(2, Math.floor((VW - 1.5) / cw)), rows = Math.max(1, Math.floor((VH - 1.5) / rh));
  // Espectador detallado de frente (cabeza con pelo/cara, hombros, torso, brazos; algunos festejando con los brazos en alto).
  const spec = (x, y, sh, ha, sk, up) => {
    const arms = up
      ? `<rect x="${x + 0.1}" y="${y + 0.9}" width="0.95" height="3.1" rx="0.45" fill="${sh}"/><rect x="${x + 0.1}" y="${y + 0.3}" width="0.95" height="1" rx="0.45" fill="${sk}"/><rect x="${x + 3.45}" y="${y + 0.9}" width="0.95" height="3.1" rx="0.45" fill="${sh}"/><rect x="${x + 3.45}" y="${y + 0.3}" width="0.95" height="1" rx="0.45" fill="${sk}"/>`
      : `<rect x="${x + 0.05}" y="${y + 3.2}" width="0.95" height="2.6" rx="0.45" fill="${sh}"/><rect x="${x + 0.05}" y="${y + 5.4}" width="0.95" height="0.9" rx="0.45" fill="${sk}"/><rect x="${x + 3.5}" y="${y + 3.2}" width="0.95" height="2.6" rx="0.45" fill="${sh}"/><rect x="${x + 3.5}" y="${y + 5.4}" width="0.95" height="0.9" rx="0.45" fill="${sk}"/>`;
    const torso = `<rect x="${x + 0.6}" y="${y + 3}" width="3.3" height="3.5" rx="1" fill="${sh}"/><rect x="${x + 0.6}" y="${y + 3.9}" width="3.3" height="0.6" fill="#fff" fill-opacity=".4"/>`;
    const head = `<rect x="${x + 1.05}" y="${y + 0.4}" width="2.5" height="2.7" rx="1.1" fill="${sk}"/><rect x="${x + 0.8}" y="${y}" width="3" height="1.6" rx="0.9" fill="${ha}"/><rect x="${x + 0.8}" y="${y + 1.1}" width="0.55" height="1.1" rx="0.25" fill="${ha}"/><rect x="${x + 3.25}" y="${y + 1.1}" width="0.55" height="1.1" rx="0.25" fill="${ha}"/><rect x="${x + 1.55}" y="${y + 1.6}" width="0.55" height="0.65" rx="0.25" fill="#2a2a2a"/><rect x="${x + 2.55}" y="${y + 1.6}" width="0.55" height="0.65" rx="0.25" fill="#2a2a2a"/><rect x="${x + 1.85}" y="${y + 2.45}" width="0.9" height="0.4" rx="0.2" fill="#c98b7a"/>`;
    return arms + torso + head;
  };
  let benches = '', s = '', idx = 0;
  for (let r = 0; r < rows; r++) benches += `<rect x="0" y="${1.4 + r * rh}" width="${VW}" height="${rh - 1.2}" fill="#566069"/><rect x="0" y="${1.4 + r * rh}" width="${VW}" height="0.9" fill="#454e57"/>`;
  for (let r = rows - 1; r >= 0; r--) for (let c = 0; c < cols; c++) { idx++;
    if (((c * 3 + r * 7 + idx) % 8) < 3) continue; // butaca vacía → ocupación variada
    const x = 0.8 + c * cw, y = 0.8 + r * rh, sh = shirts[(idx * 5 + r) % shirts.length], ha = hairs[idx % hairs.length], sk = skin[(idx + r) % skin.length], up = ((idx * 5 + c) % 5 === 0), d = ((idx * 0.41) % 2.4).toFixed(2);
...
function gymSimpleHtml(layout, t) {
  ...
  for (let i = 0; i < n; i++) { const num = i + 1, o = occ[num];
    if (o) _gymTipData[num] = gymUnitDetail(o.unit); // detalle para el modal al tocar la tarjeta
    cards += `<div class="gs-card ${o ? 'busy tappable' : 'free'}"${o ? ` onclick="gymCardDetail(${num})"` : ''}><div class="gs-num">Mesa ${num}${o ? ' <span class="gs-i">ⓘ</span>' : ''}</div>${o ? `<div class="gs-occ"><span class="g-live-dot"></span>${esc(o.title)}</div>` : `<div class="gs-free">Libre</div>`}</div>`; }
  return `<div class="gym-simple">${cards || '<div class="empty">Este gimnasio no tiene mesas configuradas.</div>'}</div>`;
}
// Detalle de la mesa (en la vista simplificada): abre un modal con los partidos de la zona y sus resultados,
// o quién está jugando la llave (el mismo contenido que el tooltip de la vista de gimnasio).
function gymCardDetail(num) {
  const html = _gymTipData[num]; if (!html) return;
  openModal(`<div class="gtip-modal">${html}</div><div class="row" style="margin-top:14px;justify-content:flex-end"><button class="btn btn-ghost" onclick="closeModal()">Cerrar</button></div>`);
}
...
// Vista de TRANSMISIÓN EN VIVO: el reproductor de YouTube con SUS controles nativos (pausar, "volver al vivo"
// con el indicador EN VIVO, pantalla completa). Lo único que agregamos es que, al entrar en pantalla completa
// en el celular, se bloquee la orientación HORIZONTAL para verlo completo (listener global, más abajo).
// + (admin/colaboradores) el botón para cargar/cambiar el link.
function gymLiveHtml(url, canEdit) {
  ...
  const src = `https://www.youtube.com/embed/${id}?rel=0&playsinline=1&autoplay=1&mute=1`;
  return `<div class="gym-live"><div class="gym-live-wrap"><iframe class="gym-live-frame" src="${src}" title="Transmisión en vivo" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div>${setBtn}</div>`;
}
// Al entrar en PANTALLA COMPLETA (con el botón nativo de YouTube), bloquear la orientación HORIZONTAL en el
// celular para que el video se vea completo; al salir, liberarla. (En iOS el sistema no permite el lock → usa
// su pantalla completa nativa, que igual rota al girar el teléfono.) Único uso de fullscreen en la app.
if (typeof document !== 'undefined' && !window.__gymFsLock) {
  window.__gymFsLock = true;
  const onFs = () => {
    const fs = document.fullscreenElement || document.webkitFullscreenElement;
    try {
      if (fs && screen.orientation && screen.orientation.lock) screen.orientation.lock('landscape').catch(() => {});
      else if (!fs && screen.orientation && screen.orientation.unlock) screen.orientation.unlock();
    } catch (e) {}
  };
